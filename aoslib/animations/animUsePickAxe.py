...
class AnimUsePickAxe(Animation):
    default_length = 0.2

    def __init__(self, length=default_length, speed=None):
        super(AnimUsePickAxe, self).__init__(length, speed)

    def start(self, length=None):
        super(AnimUsePickAxe, self).start(length)
        self.position = Vector3(0.0, 0.0, 0.0)
        self.orientation = Vector3(0.0, 0.0, 0.0)

    def update(self, dt):
        dt = min(self.timer, dt * self.speed)
        if super(AnimUsePickAxe, self).update(dt):
