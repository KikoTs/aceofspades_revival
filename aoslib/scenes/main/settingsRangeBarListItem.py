...
from aoslib.scenes.gui.rangeBarControl import RangeBarControl
from pyglet import gl
from shared.hud_constants import UI_CONTROL_SPACING, TEXT_BACKGROUND_SPACING

class SettingsRangeBarListItem(SettingsListItemBase):

    def initialize(self, name, id, initial_value, on_value_changed_callback):
        super(SettingsRangeBarListItem, self).initialize(name)
        self.id = id
        self.initial_value = initial_value
        self.control = RangeBarControl(initial_value, self.x1, self.y1, self.width, self.height)
        self.control.add_handler(on_value_changed_callback)
        self.control.fire = True
        self.elements.append(self.control)

    def set_enabled(self, enabled):
        super(SettingsRangeBarListItem, self).set_enabled(enabled)
    ...
    def update_position(self, x, y, width, height, highlight_width):
        super(SettingsRangeBarListItem, self).update_position(x, y, width, height, highlight_width)
