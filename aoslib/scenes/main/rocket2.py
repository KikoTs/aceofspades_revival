...
class Rocket2(Entity):
    ...
    def delete(self):
        ...
        if pos.z >= MAP_Z - 1:
            self.scene.media.play_pitched(ROCKET2_WATER_EXPLODE_SOUND, pos=(pos.x, pos.y, pos.z), zone=media.IN_WORLD_AUDIO_ZONE)
        else:
            self.scene.media.play_pitched(ROCKET2_EXPLODE_SOUND, pos=(pos.x, pos.y, pos.z), zone=media.IN_WORLD_AUDIO_ZONE)
        if self.sound:
            self.sound.close()
